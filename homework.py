...
def process_directory(src_dir: Path, target_dir: Path, executor):
    futures = []
    ...
    for entry in src_dir.iterdir():
        if entry.is_file():
            ...
            futures.append(executor.submit(process_directory, entry, target_dir, executor))
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = process_directory(src_path, dst_path, executor)
        wait(futures)
